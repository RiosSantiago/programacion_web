...
const connectionString = (import.meta.env.DATABASE_URL || process.env.DATABASE_URL || '').trim();

if (!connectionString) {
  throw new Error('[db] DATABASE_URL no definido en .env. La aplicación no puede iniciar.');
}
...
export interface DbTransactionClient {
  queryAll: <T = any>(sql: string, params?: any[] | Record<string, any>) => Promise<T[]>;
  queryGet: <T = any>(sql: string, params?: any[] | Record<string, any>) => Promise<T | undefined>;
...
export async function withTransaction<T>(
  ...
  try {
    ...
    const txClient: DbTransactionClient = {
      queryAll: async <R = any>(sql: string, params: any[] | Record<string, any> = []): Promise<R[]> => {
        const { text, values } = buildQuery(sql, params);
        const res = await client.query<R>(text, values);
        return parseNumericFields(res.rows as any[]) as R[];
      },
      queryGet: async <R = any>(sql: string, params: any[] | Record<string, any> = []): Promise<R | undefined> => {
        const rows = await txClient.queryAll<R>(sql, params);
        return rows[0];
      },
      queryRun: async (sql: string, params: any[] | Record<string, any> = []): Promise<{ changes: number; lastInsertRowid: number }> => {
        let { text, values } = buildQuery(sql, params);
        const isInsert = /^\s*INSERT\s+/i.test(text);
        if (isInsert && !/RETURNING\s+id\b/i.test(text)) {
          text += ' RETURNING id';
      ...
      execSql: async (sql: string): Promise<void> => {
