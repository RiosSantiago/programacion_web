...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    if (!tokenPayload) return new Response(JSON.stringify({ error: 'No autorizado' }), { status: 401 });
    ...
    await withTransaction(async (tx) => {
      if (data.imagenes !== undefined && Array.isArray(data.imagenes)) {
        await tx.queryRun('DELETE FROM imagenes_producto WHERE producto_id = ?', [id]);
        for (let i = 0; i < data.imagenes.length; i++) {
          await tx.queryRun('INSERT INTO imagenes_producto (producto_id, url, orden) VALUES (?, ?, ?)', [id, data.imagenes[i], i]);
        }
      }
      if (fields.length > 0) {
        paramsArr.push(id);
        await tx.queryRun(`UPDATE productos SET ${fields.join(', ')} WHERE id = ?`, paramsArr);
      }
    });
...
export const DELETE: APIRoute = async ({ params, request }) => {
  try {
    ...
    const rol = await getUserRole(userId);
    const canDeleteAny = rol === 'root' || rol === 'admin';
    const product = canDeleteAny
