...
import { queryAll, queryGet, withTransaction } from '../../lib/db';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!tokenPayload) {
      return new Response(JSON.stringify({ error: 'Debes iniciar sesión para publicar un producto.' }), {
    ...
    const user = await queryGet<any>('SELECT id, nombre FROM usuarios WHERE id = ?', [userId]);
    const userName = user?.nombre || data.vendedor || 'Mi Hacienda';
    ...
    const newId = await withTransaction(async (tx) => {
      const { lastInsertRowid } = await tx.queryRun(
      ...
      for (let i = 0; i < imgsToInsert.length; i++) {
        await tx.queryRun('INSERT INTO imagenes_producto (producto_id, url, orden) VALUES (?, ?, ?)', [lastInsertRowid, imgsToInsert[i], i]);
      }
    ...
    return new Response(JSON.stringify({ error: 'Error interno del servidor al publicar' }), { status: 500 });
  }
};